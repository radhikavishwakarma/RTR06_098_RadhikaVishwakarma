#include<stdio.h>
#include<stdlib.h>//contains prototype of malloca( and free())
...
int main(void)
{
     ...
     * ****To Allocate said amount of memory,Fucntions malloca() will bew used 
     * 
   ...
   if(ptr_iArray == NULL)//If ptr_iArray Is still NUMM,Even after call to malloca() It menas malloc() has failed to allocate
   //memeory and no address has been returned malloca() intptr_iArrya...
   {
    printf("\n\n");
    printf("Memory Allocations For Intger Arrat Failked !!!! Existing Now...\n]n");
    exit(0);
   }
   else // Is ptr_iArray is not NUl it menas it must contain a valid address which is rturedn by malloca(),Hence,malloc() has succeded in memoryallocations..
   {
    printf("\n\n");
    printf("Memory Allocation for iNtger Aray Has succeded !!!|n\n");
