...
{
	// variable declaration
	GLfloat fovYInDegrees = 30.0f;
	GLfloat aspectRatio = (GLfloat)width / (GLfloat)height;

    GLfloat nearZ = 0.1f;
    GLfloat  farZ = 100.0f;
    GLfloat ymax = nearZ * tanf(fovYInDegrees * M_PI / 360.0f);
	GLfloat xmin = -1.0f;
	...
	perspectiveProjectionMatrix[0] = (2 * nearZ) / (xmax - xmin);
    perspectiveProjectionMatrix[1] = 0.0f;
    perspectiveProjectionMatrix[2] = 0.0f;
    perspectiveProjectionMatrix[3] = 0.0f;
    perspectiveProjectionMatrix[4] = 0.0f;
    perspectiveProjectionMatrix[5] = (2 * nearZ) / (ymax - bottom);
    perspectiveProjectionMatrix[6] = 0.0f;
    perspectiveProjectionMatrix[7] = 0.0f;
    perspectiveProjectionMatrix[8] = (xmax + xmin) / (xmax - xmin);
    perspectiveProjectionMatrix[9] = (ymax + bottom) / (ymax - bottom);
    perspectiveProjectionMatrix[10] = -(farZ + nearZ) / (farZ - nearZ);
    perspectiveProjectionMatrix[11] = -1.0f;
    perspectiveProjectionMatrix[12] = 0.0f;
    perspectiveProjectionMatrix[13] = 0.0f;
    perspectiveProjectionMatrix[14] = -(2 * farZ * nearZ) / (farZ - nearZ);
    perspectiveProjectionMatrix[15] = 0.0f;

	glMultMatrixf(perspectiveProjectionMatrix);
}
