...
void display(void)
{
	...
	// after drawing sphere we need to go back 
	glPopMatrix();
	...
	// let the earth allow to rall aroun 
	...
	// Spin / roted aroun itself
	glRotatef((GLfloat) date, 0.0f, 0.0f, 1.0f);

	// set earth polygone property
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	glColor3f(0.4f, 0.9f, 1.0f);

	// draw the sphere of earth
	gluSphere(quadric, 0.25, 20, 20);

	// two back to the origin
	glPopMatrix();
	...
	// Let Moon to revolve around the Earth
	glRotatef((float)Year, 0.0f, 1.0f, 0.0f);
	
	// Keep Moon at some distance from the Sun
	glTranslatef(2.0f, 0.0f, 0.0f);

	// Adjust the poles of Sphere of Moon
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);

	// Let the Moon spin around itself.
	glRotatef((float)hour, 0.0f, 0.0f, 1.0f);

	// set Moon's polygon property
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	// Make Moon White
	glColor3f(1.0f, 1.9f, 1.0f);
