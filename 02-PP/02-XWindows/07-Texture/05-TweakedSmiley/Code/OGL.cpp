		...
		while (XPending(gpDisplay))
		{
			...
			switch (event.type)
			{
				...
				case ConfigureNotify:  // similar to WM_SIZE
					...
					break;
				case KeyPress:
					// Get keysym for special keys (Escape, numpad, function keys)
					...
					// For character keypress (regular keyboard)
...
int initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GL_FLOAT) * 4 * 2, NULL, GL_DYNAMIC_DRAW);
	...
	// Load Textures
	if(loadGLTexture(&texture_smiley, "Smiley.bmp") == False) // To have this function execute sucessfully file should be present in the folder
	{
		fprintf(gpFile, "Loading smiley texture failed!\n");
		return(-6);
	}
...
Bool loadGLTexture(GLuint *texture, const char* imageFilePath)
{
	...
	// Upload texture generate mipmaps (preferred over deprecated gluBuild2DMipmaps)
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, imageData);
	GLenum err = glGetError();
	if (err != GL_NO_ERROR)
	{
		fprintf(gpFile, "glTexImage2D generated GL error : 0x%X\n", err);
	}
	glGenerateMipmap(GL_TEXTURE_2D);

	fprintf(gpFile, "Generated textue ID = %u\n", (unsigned int)(*texture));
...
void display(void)
{
	...
	translationMatrix = vmath::translate(0.0f, 0.0f, -6.0f); //translate rectanglesbackwards
