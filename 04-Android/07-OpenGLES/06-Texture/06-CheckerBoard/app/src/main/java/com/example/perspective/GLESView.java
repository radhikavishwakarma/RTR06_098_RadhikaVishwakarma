...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener{
    ...
    private int initialize(GL10 gl)
    {
        ...
        // Create a native buffer suitable for native IO but for Java
        byteBuffer = ByteBuffer.allocateDirect(rectangle_texcoord.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        FloatBuffer rectangleTexCoordBuffer = byteBuffer.asFloatBuffer();
        rectangleTexCoordBuffer.put(rectangle_texcoord);
        rectangleTexCoordBuffer.position(0);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, rectangle_texcoord.length * 4, rectangleTexCoordBuffer, GLES32.GL_STATIC_DRAW);
        ...
        // Load texture
        texture_smiley[0] = loadGLTexture();
    ...
    private int loadGLTexture()
    {
        ...
        // just like vbo create empty image buffer
        ByteBuffer byteBuffer = ByteBuffer.allocateDirect(64 * 64 * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        byteBuffer.put(checkImage);
        byteBuffer.position(0);

        // create bitmap image in the memory from the above buffer
    ...
    private void makeCheckImage()
    {
        ...
        for(i = 0; i < 64; i++)
        {
            for(j=0; j < 64; j++)
            {
                ...
                checkImage[(i * 64 + j) * 4 + 3] = (byte)0xFF; // alpha component (white)
            }
        }
    }
