...
HRESULT initialize(void) {
	...
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(trianglePositions);
	...
	d3d11SubResourceData.pSysMem = trianglePositions;
	
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_TrianglePositionBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_TriangleColorBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_RectanglePositionBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Failed for Position Buffer %d\n", hr);
	...
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_RectangleColorBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Failed for Color Buffer %d\n", hr);
	...
	// Set Rasterization state to disable backface culling
...
void display(void) {
	...
	// Color
	// Set Vertex Buffer here created into initialize function
	stride = sizeof(float) * 3; // 3 for x,y,z
	offset = 0;
	...
	stride = sizeof(float) * 3; // 3 for x,y,z
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_RectanglePositionBuffer, &stride, &offset);

	// Color
	// Set Vertex Buffer here created into initialize function
	stride = sizeof(float) * 3; // 3 for x,y,z
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_RectangleColorBuffer, &stride, &offset);
	...
	rotationMatrix = XMMatrixRotationY(XMConvertToRadians(angleRectangle));
	worldMatrix = rotationMatrix * translationMatrix; // order of transformation is reverse of order of multiplication
	viewMatrix = XMMatrixIdentity(); // camera is at origin looking down negative z
