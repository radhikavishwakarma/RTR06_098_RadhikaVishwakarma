...
HRESULT initialize(void) {
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_PyramidPositionBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubResourceData.pSysMem = pyramidColors;
	
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_PyramidColorBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_CubePositionBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Failed for Position Buffer %d\n", hr);
	...
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubResourceData.pSysMem = cubeColors;
	
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_CubeColorBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Failed for Color Buffer %d\n", hr);
...
void display(void) {
	...
	translationMatrix = XMMatrixTranslation(-1.9f, 0.0f, 7.0f); // move triangle away from camera
