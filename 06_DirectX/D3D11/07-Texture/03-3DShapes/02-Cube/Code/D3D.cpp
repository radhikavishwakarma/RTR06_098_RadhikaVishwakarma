...
// Texture Coordinates (expanded to match 36 position vertices, V inverted)
	...
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_position); // triangle has 3 vertices and each vertex has 3 coordinates (x,y,z)
	...
	ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
	d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT; // similar to GL_STATIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_texCoord); // triangle has 3 vertices and each vertex has 3 color components (r,g,b)
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubresourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubresourceData.pSysMem = cube_texCoord;
	...
	hr = LoadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView);  // L means string is in wide character
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
