...
ID3D11Buffer *gpID3D11Buffer_PositionPyramidBuffer = NULL;
ID3D11Buffer *gpID3D11Buffer_PositionCubeBuffer = NULL;
ID3D11Buffer *gpID3D11Buffer_TexCoordPyramidBuffer = NULL;
ID3D11Buffer *gpID3D11Buffer_TexCoordCubeBuffer = NULL;
...
HRESULT initialize(void) {
	...
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidPositions);
	...
	d3d11SubResourceData.pSysMem = pyramidPositions;
	
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_PositionPyramidBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
	d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT; // similar to GL_STATIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidTexCoord);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubResourceData.pSysMem = pyramidTexCoord;
	
	// create vertex buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_TexCoordPyramidBuffer);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	const float cubeTexCoord[] = {
		// front (two triangles) – original v flipped
	...
	ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
	d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT; // similar to GL_STATIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubePositions);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubResourceData.pSysMem = cubePositions;
	...
	ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
	d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT; // similar to GL_STATIC_DRAW
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cubeTexCoord);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	
	// Initialize subresource of buffer for vertex data
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));
	d3d11SubResourceData.pSysMem = cubeTexCoord;
	...
	if(FAILED(hr))
	{
		...
		fprintf(gpFile,"LoadD3DTexture() Failed for Stone.bmp with error code %d\n", hr);
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"LoadD3DTexture() Succeeded for Stone.bmp with error code %d\n", hr);
		fclose(gpFile);
	}
	...
	d3d11SamplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR; // linear filtering
	d3d11SamplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_WRAP; // texture coordinate outside 0 to 1 range will be wrapped
	d3d11SamplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_WRAP; // texture coordinate outside 0 to 1 range will be wrapped
	d3d11SamplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_WRAP; // texture coordinate outside 0 to 1 range will be wrapped

	hr = gpID3D11Device->CreateSamplerState(&d3d11SamplerDesc, &gpID3D11SamplerState);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"ID3D11Device::CreateSamplerState() Failed with error code %d\n", hr);
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"ID3D11Device::CreateSamplerState() Succeeded with error code %d\n", hr);
		fclose(gpFile);
	}
...
void display(void) {
	...
	UINT offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_PositionPyramidBuffer, &stride, &offset);

	// Color
	// Set Vertex Buffer here created into initialize function
	stride = sizeof(float) * 2; // 2 for u,v
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_TexCoordPyramidBuffer, &stride, &offset);
	...
	translationMatrix = XMMatrixTranslation(-1.5f, 0.0f, 5.0f); // move triangle away from camera
	XMMATRIX rotationMatrix = XMMatrixIdentity();
	rotationMatrix = XMMatrixRotationY(XMConvertToRadians(anglePyramid));
	worldMatrix = rotationMatrix * translationMatrix; // order of transformation is reverse of order of multiplication
	...
	// Transformations
	translationMatrix = XMMatrixTranslation(1.5f, 0.0f, 5.0f); // move triangle away from camera
	rotationMatrix = XMMatrixRotationY(XMConvertToRadians(angleCube));
	worldMatrix = rotationMatrix * translationMatrix; // order of transformation is reverse of order of multiplication
	...
	// draw triangle
	gpID3D11DeviceContext->Draw(18, 0);
...
void uninitialize(void) {
	...
	if (gpID3D11Buffer_TexCoordPyramidBuffer)
	{
		gpID3D11Buffer_TexCoordPyramidBuffer->Release();
		gpID3D11Buffer_TexCoordPyramidBuffer = NULL;
	}
	...
	if(gpID3D11Buffer_PositionPyramidBuffer)
	{
		gpID3D11Buffer_PositionPyramidBuffer->Release();
		gpID3D11Buffer_PositionPyramidBuffer = NULL;
	}
