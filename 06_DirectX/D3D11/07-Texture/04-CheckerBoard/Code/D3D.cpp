...
HRESULT initialize(void) {
	...
	HRESULT CreateCheckerboardTexture(ID3D11ShaderResourceView**);
	HRESULT resize(int,int);
	...
	// Create a procedural checkerboard texture resource
	// This matches the OpenGL reference checkerboard example.
	hr = CreateCheckerboardTexture(&gpID3D11ShaderResourceView);
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"CreateCheckerboardTexture() Failed with error code %d\n", hr);
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"CreateCheckerboardTexture() Succeeded with error code %d\n", hr);
		fclose(gpFile);
	}
...
HRESULT CreateCheckerboardTexture(ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	texDesc.Width = CHECKERBOARD_WIDTH;
	texDesc.Height = CHECKERBOARD_HEIGHT;
	texDesc.MipLevels = 1;
	texDesc.ArraySize = 1;
	...
	subresourceData.pSysMem = checkerboard;
	subresourceData.SysMemPitch = CHECKERBOARD_WIDTH * 4;
	subresourceData.SysMemSlicePitch = 0;

	ID3D11Texture2D* pTexture = NULL;
	hr = gpID3D11Device->CreateTexture2D(&texDesc, &subresourceData, &pTexture);
	if (FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateTexture2D() Failed for checkerboard texture %d", hr);
		fclose(gpFile);
		return hr;
	}
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateShaderResourceView() Failed for checkerboard texture %d", hr);
		fclose(gpFile);
		pTexture->Release();
		return hr;
	}
...
HRESULT resize(int width, int height) {
	...
	d3d11DepthStencilViewDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D; // non-multisampled texture
