...
HRESULT initialize(void) {
	...
	const char* vertexShaderSourceCode =
		...
		"{\n" \
			...
			"{\n" \
				...
				"float3 transformedNormal = normalize(mul((float3x3)viewMatrixMulWorldMatrix, norm.xyz));\n" \
				"float3 lightSource = normalize((lightPosition - eyeCoordinates).xyz);\n" \
				"float transformedNormalDotLightSource = saturate(dot(transformedNormal, lightSource));\n" \
				"float4 ambient = la * ka;\n" \
				"float4 diffuse = ld * kd * transformedNormalDotLightSource;\n" \
	...
	const char* pixelShaderSourceCode =
		...
		"{\n" \
			"float4 color = input.phongADSColor;\n" \
			"return(color);\n" \
		"}\n";
