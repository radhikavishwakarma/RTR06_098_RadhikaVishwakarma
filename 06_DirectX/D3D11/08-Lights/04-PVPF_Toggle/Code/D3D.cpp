...
FLOAT materialShininess[] = {128.0f};	// [0,128]
...
struct CBUFFER
{
	...
	XMVECTOR La;	// Ambient light color
	XMVECTOR Ld;	// Diffuse light color
	XMVECTOR Ls;	// Specular light color
	XMVECTOR Ka;	// Ambient material color
	XMVECTOR Kd;	// Diffuse material color
	XMVECTOR Ks;	// Specular material color
...
HRESULT initialize(void) {
	...
	const char* vertexShaderSourceCode =
		...
		"{\n" \
			...
			"float3 eyeCoordinates : TEXCOORD0;\n" \
		...
		"{\n" \
			...
			"float3 transformedNormal = normalize(mul((float3x3)worldViewMatrix, norm));\n" \
			"float3 lightDirection = normalize((lightPosition - eyeCoordinates).xyz);\n" \
			...
			"float4 diffuse = ld * kd * saturate(dot(transformedNormal, lightDirection));\n" \
			"float4 specular = ls * ks * pow(saturate(dot(reflectionVector, viewerVector)), materialShininess);\n" \
			"output.phongADSColor = float4(1.0, 1.0, 1.0, 1.0);\n" \
			"if(lKeyPressed == 1 && PerVertex == 1)\n" \
			"{\n" \
				"output.phongADSColor = ambient + diffuse + specular;\n" \
			"}\n" \
			"output.transformedNormal = transformedNormal;\n" \
			"output.eyeCoordinates = eyeCoordinates.xyz;\n" \
	...
	const char* pixelShaderSourceCode =
		...
		"{\n" \
			...
			"if(PerVertex == 1)\n" \
			"{\n" \
				"color = input.phongADSColor;\n" \
			"}\n" \
			"else if(PerFragment == 1 && lKeyPressed == 1)\n" \
			"{\n" \
				"float3 lightDirection = normalize((lightPosition - float4(input.eyeCoordinates, 1.0f)).xyz);\n" \
				"float3 normal = normalize(input.transformedNormal);\n" \
				"float3 viewerVector = normalize(-input.eyeCoordinates);\n" \
				"float3 reflectionVector = reflect(-lightDirection, normal);\n" \
				"float4 ambient = la * ka;\n" \
				"float4 diffuse = ld * kd * saturate(dot(normal, lightDirection));\n" \
				"float4 specular = ls * ks * pow(saturate(dot(reflectionVector, viewerVector)), materialShininess);\n" \
	...
	ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
	d3d11BufferDesc.Usage = D3D11_USAGE_DEFAULT; // similar to GL_STATIC_DRAW
	d3d11BufferDesc.ByteWidth = gNumVertices * 3 * sizeof(float); // sphere has gNumVertices vertices and each vertex has 3 coordinates (x,y,z)
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	...
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Failed for Index Buffer %d\n", hr);
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"ID3D11Device::CreateBuffer() Succeeded for Index Buffer %d\n", hr);
		fclose(gpFile);
	}
	...
	// Set constant buffer into both shader pipelines
...
HRESULT resize(int width, int height) {
	...
	d3d11DepthStencilViewDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2DMS; // multisampled texture (MSAA)
...
void display(void) {
	...
	// Set Index Buffer
	gpID3D11DeviceContext->IASetIndexBuffer(gpID3D11Buffer_IndexBuffer, DXGI_FORMAT_R16_UINT, 0); // 16 bit unsigned int
	...
	rotationMatrixX = XMMatrixRotationX(XMConvertToRadians(angleSphere));
	rotationMatrixY = XMMatrixRotationY(XMConvertToRadians(angleSphere));	
	rotationMatrixZ = XMMatrixRotationZ(XMConvertToRadians(angleSphere));
	...
	constantBuffer.PerVertex = bPerVertex ? 1 : 0;
	constantBuffer.PerFragment = bPerFragment ? 1 : 0;
	if(bLight == TRUE)
	{
		...
		constantBuffer.LightPosition = XMVector3TransformCoord(
			XMVectorSet(lightPosition[0], lightPosition[1], lightPosition[2], lightPosition[3]),
			viewMatrix
		);
	}
	else
