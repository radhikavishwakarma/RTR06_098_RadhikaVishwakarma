...
HRESULT initialize(void) {
	...
	const char* geometryShaderSourceCode =
		...
		"};\n" \
		"[maxvertexcount(9)]\n" \
		"void main(triangle vertex_output input[3], inout TriangleStream<geometry_output> triangle_stream)\n" \
		"{\n" \
	...
	// 2. Compile the shader programmatically
	ID3DBlob *pID3DBlobGeometryShaderCode = NULL;
	pID3DBlob_error = NULL;
	hr = D3DCompile(
	...
	if(FAILED(hr))
	{
		...
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile,"Geometry Shader Compilation Failed : %s \n", (char*)pID3DBlob_error->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_error->Release();
			pID3DBlob_error = NULL;
		}
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"Geometry Shader Compilation Succeeded \n");
		fclose(gpFile);
	}

	// 3. Create the Geometry Shader
	hr = gpID3D11Device->CreateGeometryShader(
			pID3DBlobGeometryShaderCode->GetBufferPointer(),
			pID3DBlobGeometryShaderCode->GetBufferSize(),
			NULL,
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateGeometryShader() Failed for %d\n", hr);
		pID3DBlobGeometryShaderCode->Release();
		pID3DBlobGeometryShaderCode = NULL;
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile,"ID3D11Device::CreateGeometryShader() Succeeded for %d\n", hr);
		fclose(gpFile);
	}
	
	// 4.  set geometry shader into pipeline
	...
	d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(triangle_position); // triangle has 3 vertices and each vertex has 3 coordinates (x,y,z)
...
void display(void) {
	...
	// Draw triangle
	gpID3D11DeviceContext->Draw(3, 0); // 3 vertices, starting from 0th vertex
