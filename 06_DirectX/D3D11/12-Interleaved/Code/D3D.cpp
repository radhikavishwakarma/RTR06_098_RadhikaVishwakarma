...
HRESULT initialize(void) {
	...
	const float cube_texCoord[] = {
		// front (6 vertices: TR, TL, BL, TR, BL, BR)
		1.0f, 1.0f,
		0.0f, 1.0f,
		0.0f, 0.0f,
		1.0f, 1.0f,
	...
	hr = LoadD3DTexture(L"marble.bmp", &gpID3D11ShaderResourceView);  // L means string is in wide character
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"LoadD3DTexture() Failed for marble.bmp with error code %d\n", hr);
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");	// if file doesn't exist creates it
		fprintf(gpFile,"LoadD3DTexture() Succeeded for marble.bmp with error code %d\n", hr);
		fclose(gpFile);
	}
